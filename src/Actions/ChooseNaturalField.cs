...
namespace Trestlebridge.Actions
{
    ...
    {
        public static void CollectInput(Farm farm, INaturalFieldSeed seed)
        {
            Console.Clear();

            List<NaturalField> AvailableFields = new List<NaturalField>();

            for (int i = 0; i < farm.NaturalFields.Count; i++)
            {
                if (farm.NaturalFields[i].GetCount < farm.NaturalFields[i].Capacity)
                {
                    AvailableFields.Add(farm.NaturalFields[i]);
                }
            }

            if (AvailableFields.Count == 0)
            {
                Console.WriteLine("There are no natural fields with enough space available! Please create another plowed field. Press any key to continue.");
                Console.Read();
            }
...

            {
                for (int i = 0; i < AvailableFields.Count; i++)
                {
                    ...
                    Console.WriteLine($"{i + 1}. Natural Field ({AvailableFields[i].GetCount} seeds),");
